...
public class LiquidProgressUI extends BasicProgressBarUI {
	...
	private void createAnimation(Graphics2D graphcs, int x, int y, int size) {	
		...
		g2.fill(new ModelLiquid(
				new Rectangle((int) (location1 * size), size - value - waterStyleHeight, size, waterStyleHeight))
				.createWaterStyle());
		g2.fill(new ModelLiquid(
				new Rectangle((int) (location2 * size), size - value - waterStyleHeight, size, waterStyleHeight))
				.createWaterStyle());
		g2.dispose();
		graphcs.drawImage(img, x, y, null);
	}
	private void paintString(Graphics g) {
		Insets b=progressBar.getInsets();
		int BarRectWidth=progressBar.getWidth()-b.right-b.left;
		int barRectHeigth=progressBar.getHeight()-b.top-b.bottom;
		g.setColor(progressBar.getForeground());
		paintString(g, b.left,b.top, BarRectWidth, barRectHeigth, 0, b);
	}
}
